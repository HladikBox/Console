...
////以下是代码开始，请勿删除此行注释
////starthere
$longitude=$_REQUEST["lng"]+0;
$latitude=$_REQUEST["lat"]+0;
$sql="
select cities.id, provinces.provinceid,provinces.province,
cities.cityid,cities.city,
areas.areaid,areas.area,
areas.longitude,areas.latitude,
concat(provinces.province,cities.city,areas.area) provincecityarea,now() updated_date from provinces 
inner join cities on provinces.provinceid=cities.provinceid
inner join (select  areas.* ,
(longitude-$longitude)*(longitude-$longitude)+(latitude-$latitude)*(latitude-$latitude) distance 
from areas order by distance limit 0,1) areas on cities.cityid=areas.cityid
