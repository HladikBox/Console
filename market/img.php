 ...
 */
  ...
  $path=ROOT."/submit_apps/".$appinfo["app_id"]."/product/".encode($_REQUEST["product"])."/imgs/".encode($_REQUEST["name"]);
  if(!file_exists($path)){
    echo "no file";
    exit;
  }
  RedirectDownload(encode($_REQUEST["name"]),$path,false,"image/jpg");
?>